...
// *** MODIFICATION: Simplified to one state ***
const int NUM_OF_STATES = 1;
char* theStates[] = { "0 Pull Crank" };
...
// *** MODIFICATION: Both buttons now select state 0 ***
void countDown() {
  data.stateNumber = 0;
  updateLCD();
}
...
void updateCrankPull() {
  ...
  // Check if enough time has passed since the last crank move
...
void loop() {
  // *** MODIFICATION: Call the crank update function on every single loop ***
  updateCrankPull();
  ...
  if (radio.available(&pipeNum)) {
    ...
    switch (data.stateNumber) {
      // *** MODIFICATION: Crank logic moved to case 0 ***
      case 0:
        if (isCrankPulling) {
          // If we get a "0" command but are already pulling, ignore it.
          Serial.println(F("...but crank is already pulling. Command ignored."));
        } else {
